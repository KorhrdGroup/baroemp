  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <Link href="/admin/users" className="text-label-2 text-brand-blue-600 hover:underline">
            ← 회원 목록
          </Link>
          <h1 className="mt-1 text-title-3 font-bold text-slate-900">{profile.name}</h1>
          <p className="mt-1 flex flex-wrap items-center gap-2 text-label-1 text-slate-500">
            <span>
            ...
            {profile.marketingConsent ? (
              <Badge className="rounded-md border-0 bg-brand-blue-50 text-label-2 text-brand-blue-700">마케팅 수신 동의</Badge>
            ) : (
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <SectionCard title="A. 기본정보">
          <dl className="grid grid-cols-2 gap-3 text-label-1">
            <div>
        ...
        <SectionCard title="B. Lead">
          {lead ? (
            <div className="space-y-3 text-label-1">
              <div className="flex items-center gap-3">
                <Badge className={cn("rounded-md border-0 px-2 font-bold", gradeClass(lead.score.grade))}>
                  {lead.score.grade}
                </Badge>
                <span className="text-title-2 font-bold text-slate-900">{lead.score.totalScore}</span>
              ...
              <div className="space-y-1 rounded-lg bg-slate-50 p-3">
                <p className="text-label-2 font-semibold text-slate-500">점수 상세 이유</p>
                {lead.score.signals
                  .filter((s) => s.active)
                  .map((s) => (
            ...
            <p className="text-label-1 text-slate-500">Lead 정보 없음</p>
          )}
        </SectionCard>
        ...
        <SectionCard title="C. Career Profile">
          {careerProfile ? (
            ...
            <p className="text-label-1 text-slate-500">Career Profile 없음</p>
          )}
        </SectionCard>
        ...
        <SectionCard title="D. 관심">
          <div className="space-y-3 text-label-1">
            <div>
              <p className="mb-1 text-label-2 text-slate-400">관심직업</p>
            ...
            <div>
              <p className="mb-1 text-label-2 text-slate-400">보유자격</p>
              <div className="flex flex-wrap gap-1">
            ...
            <div>
              <p className="mb-1 text-label-2 text-slate-400">관심자격</p>
              <div className="flex flex-wrap gap-1">
            ...
            <div>
              <p className="mb-1 text-label-2 text-slate-400">관심콘텐츠</p>
      ...
      <SectionCard title="채용 관심 (Job Behavior)">
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">최근 7일 공고조회</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.jobBehavior.recentViewCount}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">찜</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.jobBehavior.bookmarkCount}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">지원클릭</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.jobBehavior.applyClickCount}</p>
          </div>
        </div>

        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <p className="mb-1.5 text-label-2 font-semibold text-slate-500">TOP 관심직업 (실제 조회/찜/지원 기반)</p>
            {detail.jobBehavior.topInterestOccupations.length === 0 ? (
              <p className="text-label-1 text-slate-400">아직 데이터가 없습니다.</p>
            ) : (
              <ul className="space-y-1 text-label-1">
          ...
          <div>
            <p className="mb-1.5 text-label-2 font-semibold text-slate-500">TOP 세부관심</p>
            <div className="flex flex-wrap gap-1.5">
              {detail.jobBehavior.topDetailTags.length === 0 ? (
                <span className="text-label-1 text-slate-400">아직 데이터가 없습니다.</span>
              ) : (
          ...
          <div>
            <p className="mb-1.5 text-label-2 font-semibold text-slate-500">검색 키워드</p>
            <div className="flex flex-wrap gap-1.5">
              {detail.jobBehavior.searchKeywords.length === 0 ? (
                <span className="text-label-1 text-slate-400">없음</span>
              ) : (
          ...
          <div>
            <p className="mb-1.5 text-label-2 font-semibold text-slate-500">지역 선호</p>
            <div className="flex flex-wrap gap-1.5">
      ...
      <SectionCard title="지원제도 관심 (Support Behavior)">
        <div className="grid gap-4 sm:grid-cols-4">
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">지원금 검사</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">
              {detail.supportBehavior.hasCompletedAssessment ? "완료" : "미완료"}
            </p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">높은 가능성 제도</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.supportBehavior.highEligibilityCount}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">최근 조회</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.supportBehavior.recentViewCount}</p>
          </div>
          <div className="rounded-lg bg-slate-50 p-3 text-center">
            <p className="text-label-2 text-slate-400">찜</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.supportBehavior.bookmarkCount}</p>
          </div>
        </div>

        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <p className="mb-1.5 text-label-2 font-semibold text-slate-500">TOP 관심유형</p>
          ...
          <div>
            <p className="mb-1.5 text-label-2 font-semibold text-slate-500">TOP 관심제도</p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 text-label-2">
        ...
        <SectionCard title="검사 결과 (내게 맞는 직업 찾기)">
          <div className="space-y-5">
            {detail.assessmentResults.slice(0, 3).map((result, idx) => (
              <div key={result.id} className={cn("rounded-xl p-4", idx === 0 ? "bg-brand-blue-50/60" : "bg-slate-50")}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-label-1 font-semibold text-slate-500">
                ...
                <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-5">
                  {result.recommendations.slice(0, 5).map((rec, i) => (
                    <div key={rec.occupationId} className="rounded-lg bg-white p-3 ring-1 ring-slate-100">
                      <p className="text-label-2 text-slate-400">TOP{i + 1}</p>
                      <p className="text-label-1 font-semibold text-slate-800">{rec.occupationName}</p>
                      <p className="mt-1 text-label-1 font-bold text-brand-blue-600">{rec.totalScore}점</p>
                      <p className="text-label-2 text-slate-400">준비도 {rec.readinessScore}점</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {detail.assessmentResults.length > 3 && (
              <p className="text-label-2 text-slate-400">외 {detail.assessmentResults.length - 3}건의 검사 이력이 더 있습니다.</p>
            )}
          </div>
        </SectionCard>
      )}
      ...
      <SectionCard title="E. 행동 (Activity Timeline)">
        <div className="space-y-2">
          {detail.activities.slice(0, 15).map((event) => (
            <div
              key={event.id}
              className="flex items-start justify-between gap-3 border-b border-slate-50 pb-2 text-label-1 last:border-0"
            >
              <div>
              ...
              <time className="shrink-0 text-label-2 text-slate-400">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {(
          ...
          <SectionCard key={title} title={title}>
            <div className="space-y-2 text-label-1">
              ...
              {items.map((item) => (
                <div key={item.id} className="rounded-lg bg-slate-50 p-3">
                  ...
                  <ul className="mt-1 text-label-2 text-slate-500">
      ...
      >
        {detail.consultations.length === 0 ? (
          <p className="text-label-1 text-slate-500">상담 이력 없음</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {detail.consultations.map((c) => (
                <TableRow key={c.id} className="text-label-1">
      ...
      >
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div>
            <p className="text-label-2 text-slate-400">이력서 수</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.resumeSummary.resumeCount}</p>
          </div>
          <div>
            <p className="text-label-2 text-slate-400">자소서 수</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">{detail.resumeSummary.coverLetterCount}</p>
          </div>
          <div>
            <p className="text-label-2 text-slate-400">대표이력서 완성도</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">
              {detail.resumeSummary.primaryResume ? `${detail.resumeSummary.primaryResume.completeness}%` : "-"}
            </p>
          </div>
          <div>
            <p className="text-label-2 text-slate-400">최근 AI 첨삭일</p>
            <p className="mt-1 text-title-3 font-bold text-slate-800">
        ...
        {detail.resumeSummary.primaryResume ? (
          <div className="mt-4 rounded-lg bg-slate-50 p-3 text-label-1">
          ...
          <p className="mt-4 text-label-1 text-slate-500">아직 작성한 이력서가 없습니다.</p>
        )}
      </SectionCard>

      <SectionCard title="I. Career Gap (취업 준비도)">
        {detail.careerGapSummaries.length === 0 ? (
          <p className="text-label-1 text-slate-500">아직 취업 준비도 분석 이력이 없습니다.</p>
        ) : (
          <div className="space-y-3">
            {detail.careerGapSummaries.map((summary) => (
              <div key={summary.id} className="rounded-lg bg-slate-50 p-3 text-label-1">
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-slate-800">
                    {summary.occupationName ?? "직업 미지정"}
                    {summary.destinationName ? ` · ${summary.destinationName}` : ""}
                  </p>
                  <span className="text-body-1 font-bold text-brand-blue-600">{summary.readinessScore}점</span>
                </div>
                <p className="mt-1 text-slate-500">
