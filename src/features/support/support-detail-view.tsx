  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-1.5">
        ...
        {match && (
          <Badge className={cn("rounded-full border-0 text-[11px] font-semibold text-white", GRADE_BADGE_CLASS[match.grade])}>
      ...
      <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">{program.title}</h1>
      ...
      {program.requiredDocuments && program.requiredDocuments.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {program.requiredDocuments.map((doc) => (
            <span key={doc} className="rounded-full bg-slate-100 px-3 py-1 text-[13px] font-medium text-slate-600">
              {doc}
            </span>
          ))}
        </div>
      ...
      <div className="mt-8">
        <h2 className="text-lg font-bold text-slate-900">사업 상세</h2>
        <p className="mt-3 whitespace-pre-line text-[15px] leading-7 text-slate-600">{program.description}</p>
      ...
      {match ? (
        <div className="mt-8 rounded-2xl border border-border bg-white p-6">
          <h2 className="text-lg font-bold text-slate-900">내 조건과 비교</h2>
        ...
        !hasMatchSignal && (
          <div className="mt-8 rounded-2xl border border-dashed border-border bg-brand-blue-50/40 p-6 text-center">
            <p className="text-[14px] text-slate-600">지원금 진단을 받으면 이 제도와 내 조건을 비교해볼 수 있어요.</p>
            <Link
              href="/support"
      ...
      {recommendedContents.length > 0 && (
        <div className="mt-8 rounded-2xl border border-border bg-white p-6">
          <h2 className="text-lg font-bold text-slate-900">함께 준비할 수 있는 과정</h2>
          <p className="mt-1 text-[13px] text-slate-400">이 지원제도와 관련된 자격/교육 과정이에요.</p>
