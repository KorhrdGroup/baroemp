  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <Link href="/admin/contents" className="text-label-2 text-brand-blue-600 hover:underline">
            ← 콘텐츠 목록
          </Link>
          <h1 className="mt-1 text-title-3 font-bold text-slate-900">{content.title}</h1>
      ...
      <div className="rounded-xl bg-white p-4 ring-1 ring-slate-200">
        <h2 className="text-label-1 font-bold">추천 Rule</h2>
        <p className="mt-1 text-label-2 text-slate-500">
          content_recommendation_rules — 관리자 설정형 (하드코딩 금지)
        </p>
        <ul className="mt-3 space-y-2 text-label-1">
          ...
          {rules.map((rule) => (
            <li key={rule.id} className="flex flex-wrap items-center gap-2 rounded-lg bg-slate-50 px-3 py-2">
              ...
              <code className="text-label-2 text-slate-600">{JSON.stringify(rule.value)}</code>
      ...
      <div id="edit">
        <h2 className="mb-3 text-body-2 font-bold text-slate-900">콘텐츠 수정</h2>
