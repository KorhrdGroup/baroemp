...
import type { JobSyncRunRepository } from "../job-sync-run-repository";
import { createAdminSupabaseClient } from "@/lib/supabase/admin";
import { throwDataSourceError } from "@/lib/data/errors";
import { unwrapList, unwrapMaybe } from "./query-helpers";

function mapRow(row: Record<string, unknown>): JobSyncRun {
  return {
    id: String(row.id),
    provider: String(row.provider),
    startedAt: String(row.started_at),
    completedAt: (row.completed_at as string | null) ?? undefined,
    status: (row.status as JobSyncRun["status"]) ?? "running",
...
function toRow(input: Partial<JobSyncRunInput>): Record<string, unknown> {
...
export function createSupabaseJobSyncRunRepository(): JobSyncRunRepository | null {
  ...
  return {
    async findAll(limit = 50) {
      const result = await client
        ...
        .limit(limit);
      const rows = unwrapList("JobSyncRunRepository.findAll", result);
    ...
    async findLatestByProvider(provider) {
      const result = await client
        ...
        .limit(1)
        .maybeSingle();
      const row = unwrapMaybe("JobSyncRunRepository.findLatestByProvider", result);
    ...
    async create(input) {
      ...
      const { data, error } = await client
        ...
        .select("*")
        .single();
      if (error || !data) throwDataSourceError("JobSyncRunRepository.create", error ?? new Error("no data returned"));
      return mapRow(data as Record<string, unknown>);
    },
    async update(id, input) {
      const result = await client.from("job_sync_runs").update(toRow(input)).eq("id", id).select("*").maybeSingle();
      const row = unwrapMaybe("JobSyncRunRepository.update", result);
      return row ? mapRow(row as Record<string, unknown>) : null;
    },
  };
}
