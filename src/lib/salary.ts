...
/**
 * 카드·상세에 쓰는 급여 문구. 예: "시급 10,320원", "월급 246만원".
 *
...
export function formatSalary(job: Pick<Job, "salaryType" | "salaryText">): string {
  ...
  if (!text) return "급여 협의";

  const [from, to] = text.split("~").map((part) => part.trim());
  const amount = to && from === to ? from : text;

  const typeLabel = job.salaryType ? SALARY_TYPE_LABELS[job.salaryType] : undefined;
  return typeLabel ? `${typeLabel} ${withThousands(amount)}` : withThousands(amount);
}
